...
def main(model_name:str, tune_trials=1, balance_data=False):
    ...
    # split dataset
    training_data_subset, testing_data_subset = train.train_test_split(df=preprocessed_data, final_year=final_year)

    if balance_data:
        #balance data
        cat_col_encoder = transforms.ColumnsOrdinalEncoder(col_names=categorical_features)
        training_data_subset = cat_col_encoder.fit_transform(training_data_subset)
        training_data_subset = transforms.balance_data(training_data_subset, label_col_name=label_col_name)
    ...
    pipeline = Pipeline(steps=[
        ...
        ('select_optimal_cols', transforms.OptimalColumnSelector(optimal_cols_path=feature_set_path))
    ])
