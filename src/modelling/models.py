...
class LogisticWrapper(WrappedModel):
    ...
    def objective_function(self, trial, X, y, transform_pipeline:Pipeline):
        ...
        #     model_pipeline.steps.append(['logistic-regressor-model', model])
        #     print(type(model_pipeline))
        #     print(model_pipeline)
        #     print(y.isnull().sum().sum())
        model_pipeline = _make_model_pipeline(transform_pipeline=transform_pipeline, model=model)
        # Using cross_val_score to get the average precision score for each fold
        scores = cross_val_score(model_pipeline, X, y, cv=5, scoring='roc_auc', error_score="raise")
    ...
    def train(self, X, y, transform_pipeline:Pipeline=None):
        ...
        # if transform_pipeline is not None:
        #     model_pipeline = sklearn.clone(transform_pipeline)
        #     model_pipeline.steps.append(['logistic-regressor-model', self.model])
        model_pipeline = _make_model_pipeline(transform_pipeline=transform_pipeline, model=self.model)
...
class XgboostWrapper(WrappedModel):
    ...
    def init_model(self):
        ...
        if self.tuned_params is not None:
            ...
            model = self.model(n_jobs=self.n_jobs, use_labael_encoder=self.use_label_encoder)
